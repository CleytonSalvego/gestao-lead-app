...
import { ArmadilhaRespostaStatusPIPEnum } from 'src/app/enums/armadilha-respostas.enum';
import { ApontamentoArmadilhaRespostaInterface, OrdemServicoItemListInterface } from 'src/app/interfaces/ordem-servico.interface';
...
@Component({
  selector: 'app-apontamento-armadilha-pip',
  templateUrl: './apontamento-armadilha-pip.page.html',
  styleUrls: ['./apontamento-armadilha-pip.page.scss'],
  standalone: true,
  imports:[
    HeaderComponent,
    NgIf,
...
export class ApontamentoArmadilhaPipPage implements OnInit {
  ...
  async loadComboStatusArmadilha(){
    for (const tl of this.enumKeys(ArmadilhaRespostaStatusPIPEnum)) {
      var item: ComboInterface ={
          codigo: tl,
          descricao: ArmadilhaRespostaStatusPIPEnum[tl]
      }
